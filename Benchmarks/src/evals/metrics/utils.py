...
def _split_raw_fields(batch):
    """Separate raw text fields (lists of strings) from tensor fields."""
    tensor_batch = {}
    raw_fields = {}
...
def eval_text_similarity(model, tokenizer, batch, generation_args):
    ...
    # Only pass raw text fields to models that support them (e.g. DD);
    # HuggingFace's generate() rejects unknown kwargs.
    gen_raw = raw_fields if hasattr(model, 'cross_tokenizer') else {}
